...
当收到的消息**包含 `任务ID: JJC-`**，且含有「请勿重复创建」或「看板已有此任务」时：

> 🚨 **这是调度系统派发的现有任务，绝对不能再 `create`！**
> 直接推进状态，不要重复建任务。

**🔴 只需执行一条 `forward` 命令（通过 Bash 工具，不是写在回复文本里）：**

1. 从消息中读取任务ID（`JJC-YYYYMMDD-NNN` 格式）

2. **用 Bash 工具执行**（替换 JJC-xxx 为实际ID，用一句话概括任务内容填写备注）：

   ```
   python3 scripts/kanban_update.py forward JJC-xxx Zhongshu "太子接旨，[一句话概括任务]，转交中书省"
   ```

   执行成功后会看到 `✅ JJC-xxx 已转交 太子 → 中书省`。

3. 可选：用 `sessions_send` 工具通知中书省

> ✅ `forward` 命令会同时更新状态和流转记录。执行成功后调度系统自动通知中书省。
...
> ✅ 任务已通过 `create` 直接建在中书省，无需再执行 `flow` 或 `forward` 命令。
